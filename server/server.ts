...
import { Action, createEmptyGame, doAction, filterCardsForPlayerPerspective, Card, Config, NameType } from "./model"
...
let gameConfig: Config = {numberOfDecks: 2, rankLimit: 10, winningPoint:21}
let displayName: NameType = "name"
const playerUserIds = [] as string[]
...
io.on('connection', client => {
  ...
  client.on("new-game", () => {
    gameState = createEmptyGame(playerUserIds, gameConfig.numberOfDecks, gameConfig.rankLimit, gameConfig.winningPoint)
  ...
  client.on("get-playerUserIds", ()=>{
    client.emit(
      "get-playerUserIds-reply",
      playerUserIds
      )
  })
  ...
  client.on("update-config", (currentConfig:Config, currentDisplayName: NameType)=>{
    ...
    //send the format of displayName to every client when it is updated
    io.emit(
      "get-displayname-reply",
      displayName
    )
      //  client.emit(
      //    "update-config-reply", 
  ...
  // client.on("get-displayname", ()=>{
  //   //send the format of displayName to a client if it request
  //     client.emit(
  //       "get-displayname-reply",
  //       displayName
  //     )
...
app.get("/api/players", async (req, res) => {
  res.status(200).json(await players.find().toArray())
})
...
client.connect().then(() => {
  ...
  Issuer.discover("http://127.0.0.1:8081/auth/realms/game/.well-known/openid-configuration").then(issuer => {
    ...
    passport.use("oidc", new Strategy(
      ...
      async (tokenSet: any, userInfo: any, done: any) => {
        ...
        if (administrator != null) {
          ...
          if(!playerUserIds.includes(_id))
            playerUserIds.push(_id)
        }
    ...
    app.get(
      ...
      //   if(req.user)
      //     playerUserIds.push(req.user.preferred_username)
      // }
    )    
